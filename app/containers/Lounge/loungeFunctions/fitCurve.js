...
export default function fitCurve(_points, maxError, progressCallback) {
  let points = _points;
...
function fitCubic(points, leftTangent, rightTangent, error, progressCallback) {
  ...
  let maxError;
  let prevErr; // Maximum fitting error
  let splitPoint;
  let prevSplit; // Point to split point set at if we need more than one curve
  let centerVector;
  let beziers; // Array of fitted Bezier curves if we need more than one curve
  let dist;
  let i;
  ...
  const u = chordLengthParameterize(points); // Parameter values for point
  ...
  const fromCenterTangent = maths.mulItems(toCenterTangent, -1); // Unit tangent vector(s) at splitPoint
...
function generateBezier(points, parameters, leftTangent, rightTangent) {
  ...
  const bezCurve = [firstPoint, null, null, lastPoint]; // Bezier curve ctl pts
...
function computeMaxError(points, bez, parameters) {
  ...
  let maxDist; // Maximum error
  let splitPoint; // Point of maximum error
  let v; // Vector from point to curve
  let i;
  let count;
  let point;
  let t;
...
const mapTtoRelativeDistances = (bez, B_parts) => {
  let B_t_curr;
  let B_t_dist = [0];
  let B_t_prev = bez[0];
  let sumLen = 0;
...
class bezier {
  ...
  static qprime(ctrlPoly, t) {
    ...
    const pB = maths.mulItems(
      maths.subtract(ctrlPoly[2], ctrlPoly[1]),
      6 * tx * t
    );
    const pC = maths.mulItems(
